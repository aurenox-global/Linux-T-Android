...
object Term {
    ...
    /** Sesión dedicada al router de modelos TNube (estilo OpenRouter). -1 = ninguna. */
    @Volatile var routerSessionId: Int = -1

    /** El usuario quiere el router activo (para relanzarlo si se cae). */
    @Volatile var routerEnabled = false
    ...
    /** ¿Sigue vivo el router de modelos? */
    fun routerAlive(): Boolean {
        val id = routerSessionId
        if (id <= 0) return false
        val s = session(id) ?: return false
        return s.alive && s.fd >= 0
    }
    ...
    /** Arranca el router de modelos TNube en su propia sesión (segundo plano). */
    fun startRouter(ctx: Context, rows: Int, cols: Int, log: (String) -> Unit): Boolean {
        if (routerAlive()) return true
        val id = openSession(ctx, rows, cols, command = "exec /usr/local/bin/tnube-router 2>&1", log = log)
        if (id <= 0) return false
        routerSessionId = id
        routerEnabled = true
        return true
    ...
    private fun pump(s: Session) {
        ...
        synchronized(lock) {
            ...
            if (s.id == routerSessionId) {
                routerSessionId = -1
                // Watchdog del router de modelos.
                val ctx = appCtx
                if (routerEnabled && ctx != null) {
                    Thread {
                        try { Thread.sleep(4000) } catch (_: Throwable) { }
                        if (routerEnabled && !routerAlive()) {
                            try { startRouter(ctx, 24, 80) { } } catch (_: Throwable) { }
                        }
                    }.also { it.isDaemon = true }.start()
                }
            }
    ...
    fun closeSession(id: Int): Int {
        val newActive = synchronized(lock) {
            ...
            if (id == routerSessionId) routerSessionId = -1
