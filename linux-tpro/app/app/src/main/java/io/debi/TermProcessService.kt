...
class TermProcessService : Service() {
    ...
    fun applyWake(on: Boolean) {
        // Wake lock: mantiene la CPU despierta (el gateway sigue procesando).
        try {
            if (on) {
                if (wakeLock == null) {
        ...
        // Wifi lock: mantiene la red activa en segundo plano (el bot pueda responder).
        try {
    ...
    /**
     * El usuario quitó la app de "recientes": NO paramos. Las sesiones (y el gateway de
     * OpenClaw) siguen vivas en segundo plano. Reforzamos el foreground para que el
     * sistema no nos degrade el oom_adj.
     */
    ...
    companion object {
        ...
        /** Activa/desactiva el wake lock del proceso (lo llama Term al cambiar sesiones). */
